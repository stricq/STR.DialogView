...
[UsedImplicitly(Reason = "Dependency Injection by Convention.")]
public class DialogTestController(DialogTestViewModel viewModel, IMessenger messenger) : IController {
    ...
    private static Task OnInitializedAsync(EventArgs ea) {
        //  MessageBox.Show("OnInitialized");

        return Task.CompletedTask;
    }

    private static Task OnLoadedAsync(RoutedEventArgs rea) {
